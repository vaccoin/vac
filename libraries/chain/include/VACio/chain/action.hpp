...
namespace VACio { namespace chain {
    ...
    *  In the context of VACio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         VAC_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         VAC_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( VACio::chain::permission_level, (actor)(permission) )
FC_REFLECT( VACio::chain::action, (account)(name)(authorization)(data) )
