...
#define N(X) VACio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<VACio::chain::name> : private hash<uint64_t> {
      typedef VACio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const VACio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, VACio::chain::name& check);
} // fc


FC_REFLECT( VACio::chain::name, (value) )
