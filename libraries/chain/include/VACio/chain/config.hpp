...
namespace VACio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(VACio);
const static uint64_t null_account_name      = N(VACio.null);
const static uint64_t producers_account_name = N(VACio.prods);
...
const static uint64_t VACio_auth_scope       = N(VACio.auth);
const static uint64_t VACio_all_scope        = N(VACio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t VACio_any_name = N(VACio.any);
const static uint64_t VACio_code_name = N(VACio.code);
...
const static VACio::chain::wasm_interface::vm_type default_wasm_runtime = VACio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace VACio::chain::config

constexpr uint64_t VAC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / VACio::chain::config::percent_100;
}

template<typename Number>
Number VAC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + VACio::chain::config::percent_100 - VACio::chain::config::percent_1)  / VACio::chain::config::percent_100;
}
