...
#include <VACCOINlib/time.hpp>
#include <VACCOINlib/asset.hpp>
#include <VACCOINlib/contract.hpp>
#include <VACCOINlib/crypto.h>

using VACCOIN::key256;
using VACCOIN::indexed_by;
using VACCOIN::const_mem_fun;
using VACCOIN::asset;
using VACCOIN::permission_level;
using VACCOIN::action;
using VACCOIN::print;
using VACCOIN::name;

class dice : public VACCOIN::contract {
   public:
      const uint32_t FIVE_MINUTES = 5*60;

      dice(account_name self)
      :VACCOIN::contract(self),
       offers(_self, _self),
       games(_self, _self),
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         VACCOIN_assert( bet.symbol == CORE_SYMBOL, "only core token allowed" );
         VACCOIN_assert( bet.is_valid(), "invalid bet" );
         VACCOIN_assert( bet.amount > 0, "must bet positive quantity" );

         VACCOIN_assert( !has_offer( commitment ), "offer with this commitment already exist" );
         require_auth( player );

         auto cur_player_itr = accounts.find( player );
         VACCOIN_assert(cur_player_itr != accounts.end(), "unknown account");
         ...
         if( matched_offer_itr == idx.end()
            ...
            accounts.modify( cur_player_itr, 0, [&](auto& acnt) {
               VACCOIN_assert( acnt.VAC_balance >= bet, "insufficient balance" );
               acnt.VAC_balance -= bet;
               acnt.open_offers++;
            });
            ...
            auto game_itr = games.emplace(_self, [&](auto& new_game){
               ...
               new_game.deadline = VACCOIN::time_point_sec(0);
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         VACCOIN_assert( offer_itr != idx.end(), "offer does not exists" );
         VACCOIN_assert( offer_itr->gameid == 0, "unable to cancel offer" );
         require_auth( offer_itr->owner );

         auto acnt_itr = accounts.find(offer_itr->owner);
         accounts.modify(acnt_itr, 0, [&](auto& acnt){
            acnt.open_offers--;
            acnt.VAC_balance += offer_itr->bet;
         });
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         VACCOIN_assert(curr_revealer_offer != idx.end(), "offer not found");
         VACCOIN_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         VACCOIN_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
         ...
         if( !is_zero(prev_reveal.reveal) ) {
            ...
            games.modify(game_itr, 0, [&](auto& game){
               ...
               game.deadline = VACCOIN::time_point_sec(now() + FIVE_MINUTES);
            });
         }
      }
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         VACCOIN_assert(game_itr != games.end(), "game not found");
         VACCOIN_assert(game_itr->deadline != VACCOIN::time_point_sec(0) && VACCOIN::time_point_sec(now()) > game_itr->deadline, "game not expired");
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            VACCOIN_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            VACCOIN_assert( is_zero(game_itr->player1.reveal), "game error");
      ...
      void deposit( const account_name from, const asset& quantity ) {
         ...
         VACCOIN_assert( quantity.amount > 0, "must deposit positive quantity" );
         ...
         action(
            ...
            N(VACCOIN.token), N(transfer),
            std::make_tuple(from, _self, quantity, std::string(""))
         ).send();

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            acnt.VAC_balance += quantity;
         });
      }
      ...
      void withdraw( const account_name to, const asset& quantity ) {
         ...
         VACCOIN_assert( quantity.is_valid(), "invalid quantity" );
         VACCOIN_assert( quantity.amount > 0, "must withdraw positive quantity" );

         auto itr = accounts.find( to );
         VACCOIN_assert(itr != accounts.end(), "unknown account");

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            VACCOIN_assert( acnt.VAC_balance >= quantity, "insufficient balance" );
            acnt.VAC_balance -= quantity;
         });
   ...
   private:
      ...
      struct offer {
         ...
         VACLIB_SERIALIZE( offer, (id)(owner)(bet)(commitment)(gameid) )
      };

      typedef VACCOIN::multi_index< N(offer), offer,
      ...
      struct player {
         ...
         VACLIB_SERIALIZE( player, (commitment)(reveal) )
      };
      ...
      struct game {
         uint64_t id;
         asset    bet;
         VACCOIN::time_point_sec deadline;
         player   player1;
         player   player2;

         uint64_t primary_key()const { return id; }

         VACLIB_SERIALIZE( game, (id)(bet)(deadline)(player1)(player2) )
      };

      typedef VACCOIN::multi_index< N(game), game> game_index;
      ...
      struct global_dice {
         ...
         VACLIB_SERIALIZE( global_dice, (id)(nextgameid) )
      };

      typedef VACCOIN::multi_index< N(global), global_dice> global_dice_index;
      ...
      struct account {
         ...
         bool is_empty()const { return !( VAC_balance.amount | open_offers | open_games ); }

         uint64_t primary_key()const { return owner; }

         VACLIB_SERIALIZE( account, (owner)(VAC_balance)(open_offers)(open_games) )
      };

      typedef VACCOIN::multi_index< N(account), account> account_index;
...
VACCOIN_ABI( dice, (offerbet)(canceloffer)(reveal)(claimexpired)(deposit)(withdraw) )
