...
namespace VACio {
   ...
   struct margin_position {
      ...
      VACLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef VACio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), VACio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      VACLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef VACio::multi_index<N(loans), loan_position> loans;
