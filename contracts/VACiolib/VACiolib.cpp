...
namespace VACCOIN {
   ...
   void set_blockchain_parameters(const VACCOIN::blockchain_parameters& params) {
      char buf[sizeof(VACCOIN::blockchain_parameters)];
      VACCOIN::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(VACCOIN::blockchain_parameters& params) {
      char buf[sizeof(VACCOIN::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      VACCOIN_assert( size <= sizeof(buf), "buffer is too small" );
      VACCOIN::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            VACCOIN_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return VACCOIN::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = VACCOIN::memory_heap.malloc(count*size);
...
{
   return VACCOIN::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return VACCOIN::memory_heap.free(ptr);
}
