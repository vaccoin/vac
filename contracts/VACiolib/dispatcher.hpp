...
namespace VACCOIN {
    ...
    * ```
    *
    * For this to work the Actions must be derived from VACCOIN::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return VACCOIN::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from VACCOIN::contract
...
// Helper macro for VACCOIN_API
#define VACCOIN_API_CALL( r, OP, elem ) \
   case ::VACCOIN::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      VACCOIN::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for VACCOIN_ABI
#define VACCOIN_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( VACCOIN_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from VACCOIN::contract
 * 
 ...
 * Example:
 * @code
 * VACCOIN_ABI( VACCOIN::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define VACCOIN_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "VACCOIN" code account and authorized by "VACCOIN"'s "active permission */ \
         VACCOIN_assert(code == N(VACCOIN), "onerror action's are only valid from the \"VACCOIN\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            VACCOIN_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: VACCOIN_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
