...
#include <VACCOINlib/system.h>
#include <VACCOINlib/symbol.hpp>
#include <tuple>
#include <limits>
...
namespace VACCOIN {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         VACCOIN_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         VACCOIN_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         VACCOIN_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         VACCOIN_assert( -max_amount <= amount, "subtraction underflow" );
         VACCOIN_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         VACCOIN_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         VACCOIN_assert( -max_amount <= amount, "addition underflow" );
         VACCOIN_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         VACCOIN_assert( a == 0 || (amount * a) / a == amount, "multiplication overflow or underflow" );
         VACCOIN_assert( -max_amount <= amount, "multiplication underflow" );
         VACCOIN_assert( amount <= max_amount,  "multiplication overflow" );
         amount *= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         VACCOIN_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      VACLIB_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         VACCOIN_assert( a.contract == b.contract, "type mismatch" );
      ...
      VACLIB_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
