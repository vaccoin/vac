...
#include <VACCOINlib/vector.hpp>

namespace identity {
   using VACCOIN::action_meta;
      ...
      public:
        ...
        using identity_base::identity_base;
         ...
         void create( const account_name creator, const uint64_t identity ) {
            ...
            VACCOIN_assert( itr == t.end(), "identity already exists" );
            VACCOIN_assert( identity != 0, "identity=0 is not allowed" );
                               ...
                               const vector<certvalue>& values )
         {
            ...
            idents_table t( _self, _self );
            VACCOIN_assert( t.find( identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : values ) {
               ...
               if (value.confidence) {
                  VACCOIN_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     VACCOIN_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
...
VACCOIN_ABI( identity::contract, (create)(certprop)(settrust) );
