 ...
 */
#include <VACiolib/action.h>
#include <VACiolib/chain.h>
...
void test_chain::test_activeprods() {
  ...
  VACio_assert(act_prods.len == 21, "producers.len != 21");

  producers api_prods;
  get_active_producers(api_prods.producers, sizeof(account_name)*21);

  for( int i = 0; i < 21 ; ++i )
      VACio_assert(api_prods.producers[i] == act_prods.producers[i], "Active producer");
}
