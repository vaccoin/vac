...
#include <VACio/bnet_plugin/bnet_plugin.hpp>
...
using VACio::public_key_type;
using VACio::chain_id_type;
using VACio::block_id_type;
using VACio::block_timestamp_type;
using std::string;
using VACio::sha256;
using VACio::signed_block_ptr;
using VACio::packed_transaction_ptr;
using std::vector;
...
namespace VACio {
     ...
     void on( const hello& hi, fc::datastream<const char*>& ds );
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              VAC_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              VAC_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           VAC_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         VAC_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
