#include <VACio/history_plugin/history_plugin.hpp>
#include <VACio/history_plugin/account_control_history_object.hpp>
#include <VACio/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(VACio::account_history_object, VACio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(VACio::action_history_object, VACio::action_history_index)
...
namespace VACio {
         ...
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodVAC to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               VAC_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               VAC_ASSERT( fe.receiver.value && fe.action.value, fc::invalid_arg_exception,
   ...
   namespace history_apis { 
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        VAC_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         if( !in_history && !p.block_num_hint ) {
            VAC_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if (in_history) {
            ...
            if (!found) {
               VAC_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }
