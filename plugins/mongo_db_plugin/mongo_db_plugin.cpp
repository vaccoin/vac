 ...
 */
#include <VACio/mongo_db_plugin/mongo_db_plugin.hpp>
               ...
               try{
                  ...
                  try {
                     if( !accounts.update_one( make_document( kvp( "_id", from_account->view()["_id"].get_oid())),
                                               update_from.view())) {
                        VAC_ASSERT( false, chain::mongo_db_update_fail, "Failed to udpdate account ${n}", ("n", setabi.account));
                     }
                  } catch( ... ) {
   ...
   try {
      ...
      if( account ) {
         ...
         try {
            ...
            ilog( "Unable to convert VAC JSON to MongoDB JSON: ${e}", ("e", e.what()));
            ilog( "  VAC JSON: ${j}", ("j", json));
      ...
      if (act.name != "onblock") { // VACio::onblock not in original VACio.system abi
   ...
   if( start_block_reached ) {
      ...
      try {
         if( !trans.insert_one( trans_doc.view())) {
            VAC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", trx_id));
         }
      } catch(...) {
      ...
      if (actions_to_write) {
         try {
            if( !bulk_actions.execute() ) {
               VAC_ASSERT( false, chain::mongo_db_insert_fail, "Bulk actions insert failed for transaction: ${id}", ("id", trx_id_str));
            }
         } catch(...) {
...
void mongo_db_plugin_impl::_process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   ...
   try {
      if( !trans_traces.insert_one( trans_traces_doc.view())) {
         VAC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", t->id));
      }
   } catch(...) {
...
void mongo_db_plugin_impl::_process_accepted_block( const chain::block_state_ptr& bs ) {
   ...
   try {
      if( !block_states.update_one( make_document( kvp( "block_id", block_id_str )),
         ...
         VAC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${bid}", ("bid", block_id));
      }
   } catch(...) {
   ...
   try {
      if( !blocks.update_one( make_document( kvp( "block_id", block_id_str )),
         ...
         VAC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${bid}", ("bid", block_id));
      }
   } catch(...) {
...
void mongo_db_plugin_impl::_process_irreversible_block(const chain::block_state_ptr& bs)
{
   ...
   if( transactions_in_block ) {
      try {
         if( !bulk.execute()) {
            VAC_ASSERT( false, chain::mongo_db_insert_fail, "Bulk transaction insert failed for block: ${bid}", ("bid", block_id));
         }
      } catch(...) {
   ...
   if (accounts.count(make_document()) == 0) {
      ...
      try {
         if( !accounts.insert_one( doc.view())) {
            VAC_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert account ${n}",
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between nodVAC and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'VAC' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/VAC")
         ;
}
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               VAC_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all VAC collections from mongodb." );
            }
         }

         if( options.count( "abi-serializer-max-time-ms") == 0 ) {
            VAC_ASSERT(false, chain::plugin_config_exception, "--abi-serializer-max-time-ms required as default value not appropriate for parsing full blocks");
         }
         ...
         VAC_ASSERT( chain_plug, chain::missing_chain_plugin_exception, ""  );
         ...
         wlog( "VACio::mongo_db_plugin configured, but no --mongodb-uri specified." );
