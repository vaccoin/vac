...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for VACio reqauth action with alice's spending key
   chain.link_authority("alice", "VACio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "VACio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with VACio reqauth
   chain.unlink_authority("alice", "VACio", "reqauth");
   ...
   // Link authority for any VACio action with alice's scud key
   chain.link_authority("alice", "VACio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "VACio", "first",  "reqauth");
...
try {
   ...
   // Creating account with VACio. prefix with privileged account
   chain.create_account("VACio.test1");

   // Creating account with VACio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("VACio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'VACio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "VACio", "VACio.any", "reqauth" );
   chain.link_authority( "bob", "VACio", "VACio.any", "reqauth" );

   /// this should succeed because VACio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = VACio::chain::resource_limits::resource_usage_object;
   using by_owner = VACio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link VACio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
