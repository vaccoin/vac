...
#include <VACio/utilities/rand.hpp>
...
BOOST_AUTO_TEST_CASE(transaction_test) { try {
   ...
   ((const signed_transaction &)trx).sign( test.get_private_key( N(VACio), "active" ), test.control->get_chain_id());
   BOOST_CHECK_EQUAL(0, trx.signatures.size());
   trx.sign( test.get_private_key( N(VACio), "active" ), test.control->get_chain_id()  );
