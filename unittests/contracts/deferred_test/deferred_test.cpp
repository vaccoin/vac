...
class deferred_test : public VACio::contract {
   public:
      ...
      void deferfunc( uint64_t payload ) {
         ...
         VACio_assert( payload != 13, "value 13 not allowed in payload" );
      }
...
extern "C" {
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == N(VACio) && action == N(onerror) ) {
...
//VACIO_ABI( deferred_test, (defercall)(deferfunc) )
