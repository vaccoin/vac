...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(VACio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(VACio.token), VACio_token_wast);
         chain->set_abi(N(VACio.token), VACio_token_abi);
         chain->push_action( N(VACio.token), N(create), N(VACio.token), mvo()
              ( "issuer", "VACio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(VACio.token), N(issue), N(VACio.token), mvo()
              ( "to", "VACio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(VACio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {N(VACio), N(VACio.token), N(alice)};
   test.init();

   test.transfer( N(VACio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(VACio), N(VACio.token), N(bob)};
   test.init();

   test.transfer( N(VACio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(VACio.token) );
   ...
   test.chain->set_code(N(bob), VACio_token_wast);
   test.chain->set_abi(N(bob), VACio_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), VACio_token_wast);
   test.chain->set_abi(N(charlie), VACio_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(VACio), N(VACio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_VACio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(N(VACio), VACio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
   tester1.contract_blacklist = {N(VACio)};
   tester1.init(false);
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   tester1.action_blacklist = {{N(VACio), N(onerror)}};
   tester1.init(false);
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'VACio::onerror' is on the action blacklist")
                        );
