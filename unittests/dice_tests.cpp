...
FC_REFLECT(account_t, (owner)(VAC_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(VACio.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(VACio.token), N(create), N(VACio.token), mvo()
     ("issuer", "VACio.token")
     ("maximum_supply", core_from_string("1000000000.0000"))
   );

   push_action(N(VACio.token), N(issue), N(VACio.token), mvo()
     ("to", "VACio")
     ("quantity", core_from_string("1000000000.0000"))
     ("memo", "")
   );

   transfer( N(VACio), N(alice), core_from_string("10000.0000"), "", N(VACio.token) );
   transfer( N(VACio), N(bob),   core_from_string("10000.0000"), "", N(VACio.token) );
   transfer( N(VACio), N(carol), core_from_string("10000.0000"), "", N(VACio.token) );
   ...
   BOOST_REQUIRE_EQUAL(
      get_currency_balance(N(VACio.token), symbol(CORE_SYMBOL), N(alice)),
