...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(VACio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(VACio.token), VACio_token_wast );
   c.set_abi( N(VACio.token), VACio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(VACio.token), N(create), N(VACio.token), mutable_variant_object()
              ("issuer",       "VACio" )
   ...
   cr = c.push_action( N(VACio.token), N(issue), N(VACio), mutable_variant_object()
              ("to",       "dan" )
