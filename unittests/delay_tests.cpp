...
#include <VACio/testing/tester_network.hpp>
...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance(N(VACio.token), symbol(SY(4,CUR)), account);
}

const std::string VACio_token = name(N(VACio.token)).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account(N(VACio.token));
   chain.produce_blocks(10);

   chain.set_code(N(VACio.token), VACio_token_wast);
   chain.set_abi(N(VACio.token), VACio_token_abi);
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action(N(VACio.token), N(create), N(VACio.token), mutable_variant_object()
           ("issuer", VACio_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action(N(VACio.token), name("issue"), N(VACio.token), fc::mutable_variant_object()
           ("to",       VACio_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(N(VACio.token), name("transfer"), N(VACio.token), fc::mutable_variant_object()
       ("from", VACio_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, N(VACio.token));
   ...
   trace = chain.push_action(N(VACio.token), name("transfer"), N(tester), fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, N(VACio.token));
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, N(tester));
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from VACio.token to tester
   trace = chain.push_action(N(VACio.token), name("transfer"), N(VACio.token), fc::mutable_variant_object()
       ("from", "VACio.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>(N(VACio.token));
   const auto abi = acnt.get_abi();
   chain::abi_serializer abis(abi, chain.abi_serializer_max_time);
   const auto a = chain.control->db().get<account_object,by_name>(N(VACio.token)).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = N(VACio.token);
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(N(VACio.token), name("transfer"), vector<permission_level>{{N(tester), N(first)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(VACio.token), name("transfer"), vector<permission_level>{{N(tester), N(second)}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(N(VACio.token), name("transfer"), vector<permission_level>{{N(tester), config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
