 ...
 * ram_tests test case
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(ram_tests, VACio_system::VACio_system_tester) { try {
   ...
   buyrambytes(N(VACio), N(VACio), 70000);
   produce_blocks(10);
   create_account_with_resources(N(testram11111),N(VACio), init_request_bytes + 40);
   create_account_with_resources(N(testram22222),N(VACio), init_request_bytes + 1190);
   produce_blocks(10);
   BOOST_REQUIRE_EQUAL( success(), stake( "VACio.stake", "testram11111", core_from_string("10.0000"), core_from_string("5.0000") ) );
   produce_blocks(10);
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         ...
         buyrambytes(N(VACio), N(testram11111), increment_contract_bytes);
         buyrambytes(N(VACio), N(testram22222), increment_contract_bytes);
      }
   }
   produce_blocks(10);
   ...
   buyrambytes(N(VACio), N(testram11111), more_ram);
   buyrambytes(N(VACio), N(testram22222), more_ram);
