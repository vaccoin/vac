...
public:
   ...
   fc::variant get_global_state() {
      vector<char> data = get_row_by_account( N(VACio), N(VACio), N(global), N(global) );
      if (data.empty()) std::cout << "\nData is empty\n" << std::endl;
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "VACio_global_state", data, abi_serializer_max_time );

   }

    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(VACio), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(VACio), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(VACio), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(VACio), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(VACio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create VACio.msig and VACio.token
        create_accounts({N(VACio.msig), N(VACio.token), N(VACio.ram), N(VACio.ramfee), N(VACio.stake), N(VACio.vpay), N(VACio.bpay), N(VACio.saving) });

        // Set code for the following accounts:
        //  - VACio (code: VACio.bios) (already set by tester constructor)
        //  - VACio.msig (code: VACio.msig)
        //  - VACio.token (code: VACio.token)
        set_code_abi(N(VACio.msig), VACio_msig_wast, VACio_msig_abi);//, &VACio_active_pk);
        set_code_abi(N(VACio.token), VACio_token_wast, VACio_token_abi); //, &VACio_active_pk);

        // Set privileged for VACio.msig and VACio.token
        set_privileged(N(VACio.msig));
        set_privileged(N(VACio.token));

        // Verify VACio.msig and VACio.token is privileged
        const auto& VACio_msig_acc = get<account_object, by_name>(N(VACio.msig));
        BOOST_TEST(VACio_msig_acc.privileged == true);
        const auto& VACio_token_acc = get<account_object, by_name>(N(VACio.token));
        BOOST_TEST(VACio_token_acc.privileged == true);


        // Create SYS tokens in VACio.token, set its manager as VACio
        ...
        create_currency(N(VACio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to VACio.system
        issue(N(VACio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(VACio) );
        }

        // Set VACio.system to VACio
        set_code_abi(N(VACio), VACio_system_wast, VACio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(VACio), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(VACio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(VACio), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "VACio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), VACio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), VACio_assert_message_exception);
