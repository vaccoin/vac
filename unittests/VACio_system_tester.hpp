...
namespace VACio_system {

class VACio_system_tester : public TESTER {
public:

   VACio_system_tester()
   : VACio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   VACio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(VACio.token), N(VACio.ram), N(VACio.ramfee), N(VACio.stake),
               N(VACio.bpay), N(VACio.vpay), N(VACio.saving), N(VACio.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(VACio.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("VACio")  + get_balance("VACio.ramfee") + get_balance("VACio.stake") + get_balance("VACio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& VACin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",VACin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(VACio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = VACio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(VACio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(VACio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "VACio", "VACio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "VACio", "alice1111111", core_from_string("650000000.0000"), "VACio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::VACio_code_name}, 1},
...
inline uint64_t M( const string& VAC_str ) {
   return core_from_string( VAC_str ).get_amount();
}
