...
#include <VACio/chain/abi_serializer.hpp>
#include <VACio/chain/VACio_contract.hpp>
#include <VACio/abi_generator/abi_generator.hpp>
...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(VACio_contract_abi(abi), max_serialization_time);
   };
}
...
{ try {
   ...
   abi_serializer abis(VACio_contract_abi(abi), max_serialization_time);
...
using namespace VACio::unittests::config;

struct abi_gen_helper {

  abi_gen_helper() {}

  static bool is_abi_generation_exception(const VACio::abi_generation_exception& e) { return true; };

  bool generate_abi(const char* source, const char* abi, bool opt_sfs=false) {

    std::string include_param = std::string("-I") + VACiolib_path;
    std::string pfr_include_param = std::string("-I") + pfr_include_path;
    std::string boost_include_param = std::string("-I") + boost_include_path;
    std::string stdcpp_include_param = std::string("-I") + VACiolib_path + "/libc++/upstream/include";
    std::string stdc_include_param = std::string("-I") + VACiolib_path +  "/musl/upstream/include";
    ...
    bool res = runToolOnCodeWithArgs(
      new find_VACio_abi_macro_action(contract, actions, ""),
      source,
      extra_args      
    );
    FC_ASSERT(res == true);
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(unknown_type, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{  try {
   ...
   #include <VACiolib/types.hpp>
   #include <VACiolib/varint.hpp>
   #include <VACiolib/asset.hpp>
   #include <VACiolib/time.hpp>
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(double_base, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(unable_to_determine_index, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(same_type_different_namespace, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(bad_index_type, "{}"), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(union_table, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(same_action_different_type, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
} FC_LOG_AND_RETHROW() }
...
{ try {
   ...
   BOOST_CHECK_EXCEPTION( generate_abi(abigen_vector_multidimension, ""), VACio::abi_generation_exception, abi_gen_helper::is_abi_generation_exception );
...
BOOST_FIXTURE_TEST_CASE(abigen_VACioabi_macro, abi_gen_helper)
{ try {

   const char* abigen_VACioabi_macro = R"=====(
      ...
      struct hello : public VACio::contract {
        public:
   ...
   const char* abigen_VACioabi_macro_abi = R"=====(
   {
     "types": [],
     "structs": [{
         "name": "hi",
   ...
   BOOST_TEST( generate_abi(abigen_VACioabi_macro, abigen_VACioabi_macro_abi) == true );
...
BOOST_FIXTURE_TEST_CASE(abigen_no_VACioabi_macro, abi_gen_helper)
{ try {

   const char* abigen_no_VACioabi_macro = R"=====(
      #pragma GCC diagnostic push
      #pragma GCC diagnostic ignored "-Wpointer-bool-conversion"
      #include <VACiolib/VACio.hpp>
      #include <VACiolib/print.hpp>
   ...
   const char* abigen_no_VACioabi_macro_abi = R"=====(
   {
     "version": "VACio::abi/1.0",
     "types": [],
     "structs": [{
   ...
   BOOST_TEST( generate_abi(abigen_no_VACioabi_macro, abigen_no_VACioabi_macro_abi) == true );
...
{ try {
   ...
   auto abi = VACio_contract_abi(fc::json::from_string(my_abi).as<abi_def>());

   abi_serializer abis(abi, max_serialization_time);

   const char *my_other = R"=====(
    {
      "publickey"     :  "VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
      "publickey_arr" :  ["VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"],
      "asset"         : "100.0000 SYS",
      ...
      "keyweight": {"key":"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},
      "keyweight_arr": [{"key":"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},{"key":"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"200"}],
      "authority": {
         "threshold":"10",
         "keys":[{"key":"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":100},{"key":"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":200}],
...
{ try {
   ...
   auto abi = VACio_contract_abi(fc::json::from_string(typedef_cycle_abi).as<abi_def>());
...
{ try {
   ...
   abi_serializer abis(VACio_contract_abi(abi_def()), max_serialization_time);
...
{ try {
   ...
   const char* test_data = R"=====(
   {
     ...
     "auth" : {
        ...
        "keys" : [ {"key" : "VAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im", "weight" : 57005},
                   {"key" : "VAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf", "weight" : 57605} ],
   ...
   BOOST_TEST("VAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)updauth.auth.keys[0].key);
   BOOST_TEST(57005u == updauth.auth.keys[0].weight);
   BOOST_TEST("VAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)updauth.auth.keys[1].key);
...
{ try {
   ...
   BOOST_TEST("VAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.owner.keys[0].key);
   BOOST_TEST(57005u == newacct.owner.keys[0].weight);
   BOOST_TEST("VAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.owner.keys[1].key);
   ...
   BOOST_TEST("VAC65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == (std::string)newacct.active.keys[0].key);
   BOOST_TEST(57005u == newacct.active.keys[0].weight);
   BOOST_TEST("VAC5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == (std::string)newacct.active.keys[1].key);
...
{ try {
   ...
   abi_serializer abis(VACio_contract_abi(v.as<abi_def>()), max_serialization_time);
...
{ try {
   ...
   auto abi = VACio_contract_abi(fc::json::from_string(repeat_abi).as<abi_def>());
...
BOOST_AUTO_TEST_CASE(abi_account_name_in_VACio_abi)
{ try {
   // inifinite loop in types
   const char* repeat_abi = R"=====(
   {
...
BOOST_AUTO_TEST_CASE(abi_recursive_structs)
{
   try {
      ...
      string hi_data = "{\"user\":\"VACio\",\"arg2\":{\"user\":\"1\"}}";
