...
my $VAC_home = defined $ENV{VAC_HOME} ? $ENV{VAC_HOME} : getcwd;
my $VACd = $VAC_home . "/programs/VACd/VACd";
my $VACc = $VAC_home . "/programs/VACc/VACc";

my $nodes = defined $ENV{VAC_TEST_RING} ? $ENV{VAC_TEST_RING} : "1";
my $pnodes = defined $ENV{VAC_TEST_PRODUCERS} ? $ENV{VAC_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$VAC_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of VACd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"VAC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = VACio::producer_plugin\n";
        print $cfg "plugin = VACio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$VACc transfer VAC inita 10 >> VACc.out 2>> VACc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
