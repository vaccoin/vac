...
killAll()
{
  programs/VACio-launcher/VACio-launcher -k 15
}

cleanup()
{
    rm -rf etc/VACio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clVAC/clVAC get info)"
  verifyErrorCode "clVAC get info"
...
# stand up nodVAC cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodVAC \"--plugin VACio::wallet_api_plugin\" $launcherOpts
programs/VACio-launcher/VACio-launcher --nodVAC "--plugin VACio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clVAC/clVAC --port $port2 get block 1)"
    verifyErrorCode "clVAC get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clVAC/clVAC create key)"
verifyErrorCode "clVAC create key"
...
PASSWORD_INITA="$(programs/clVAC/clVAC wallet create --name inita)"
verifyErrorCode "clVAC wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clVAC/clVAC wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clVAC wallet import"
programs/clVAC/clVAC wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clVAC wallet import"
programs/clVAC/clVAC wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clVAC wallet import"
programs/clVAC/clVAC wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clVAC wallet import"
...
ACCOUNT_INFO="$(programs/clVAC/clVAC create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clVAC create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clVAC/clVAC get account testera)"
verifyErrorCode "clVAC get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clVAC/clVAC transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clVAC transfer"
   ...
   TRANS_INFO="$(programs/clVAC/clVAC --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clVAC get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
