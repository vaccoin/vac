...
try:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up VAC walletd.")

    testWalletName="test"
    Print("Creating wallet \"%s\"." % (testWalletName))
    testWallet=walletMgr.create(testWalletName, [cluster.VACioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    if testWallet is None:
        cmdError("VAC wallet create")
    ...
    # create accounts via VACio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.VACioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.VACioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.VACioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.VACioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.VACioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.VACioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    actualAmount=node.getAccountVACBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountVACBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/VACio.token"
    wastFile="contracts/VACio.token/VACio.token.wast"
    abiFile="contracts/VACio.token/VACio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getVACAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get VAC account failed", raw=True)
    ...
    account=node.getVACAccount(defproduceraAccount.name)
    if account is None:
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killVACInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
