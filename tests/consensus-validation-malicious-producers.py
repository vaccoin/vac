...
plugin = VACio::account_history_plugin
plugin = VACio::account_history_api_plugin"""
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/VACio/node_%02d" % (count))
...
killVACInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up VAC cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up VAC walletd.")
            return False
