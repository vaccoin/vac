...
class TestHelper(object):
    ...
    def parse_args(includeArgs):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="nodVAC instances to kill", default=-1)
        ...
        if "--host" in includeArgs:
            parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.VACServerName),
                                     default=TestHelper.LOCAL_HOST)
        if "--port" in includeArgs:
            parser.add_argument("-p", "--port", type=int, help="%s host port" % Utils.VACServerName,
        ...
        if "--dump-error-details" in includeArgs:
            parser.add_argument("--dump-error-details",
                                     help="Upon error print etc/VACio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                                     action='store_true')
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all nodVAC and klVAC instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates nodVAC and klVAC are in path and can be started up.", action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("VAC Client version: %s" % (clientVersion))
        Utils.Print("Processor: %s" % (platform.processor()))
        Utils.Print("OS name: %s" % (platform.platform()))
    
    @staticmethod
    # pylint: disable=too-many-arguments
    def shutdown(cluster, walletMgr, testSuccessful=True, killVACInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        assert(isinstance(killVACInstances, bool))
