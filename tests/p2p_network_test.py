...
# VACio should have the same key as defproducera
VACio = copy.copy(defproduceraAccount)
VACio.name = "VACio"

Print("Info of each node:")
for i in range(len(hosts)):
    node = cluster.getNode(0)
    cmd="%s %s get info" % (testUtils.Utils.VACClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wastFile="contracts/VACio.system/VACio.system.wast"
abiFile="contracts/VACio.system/VACio.system.abi"
Print("\nPush system contract %s %s" % (wastFile, abiFile))
trans=node0.publishContract(VACio.name, wastFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish VACio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, VACio)
