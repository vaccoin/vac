...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="VACtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getVACAccount(account.name)
            try:
                assert(accountInfo)
    ...
    def getBlock(self, blockNum, silentErrors=False):
        ...
        if not self.enableMongo:
            cmd="%s %s get block %d" % (Utils.VACClientPath, self.endpointArgs, blockNum)
    ...
    def getTransaction(self, transId, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get transaction %s" % (Utils.VACClientPath, self.endpointArgs, transId)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=100):
        cmd='%s %s system newaccount -j %s %s %s %s --stake-net "%s %s" --stake-cpu "%s %s" --buy-ram "%s %s"' % (
            Utils.VACClientPath, self.endpointArgs, creatorAccount.name, account.name,
    ...
    def getVACAccount(self, name):
        assert(isinstance(name, str))
        if not self.enableMongo:
            cmd="%s %s get account -j %s" % (Utils.VACClientPath, self.endpointArgs, name)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                trans=Utils.runCmdReturnJson(cmd)
                return trans
            ...
            return self.getVACAccountFromDb(name)

    def getVACAccountFromDb(self, name):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table):
        cmd="%s %s get table %s %s %s" % (Utils.VACClientPath, self.endpointArgs, contract, scope, table)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL):
        ...
        cmd="%s %s get currency balance %s %s %s" % (Utils.VACClientPath, self.endpointArgs, contract, account, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getCurrencyStats(self, contract, symbol=CORE_SYMBOL):
        ...
        assert(symbol)
        assert(isinstance(symbol, str))
        cmd="%s %s get currency stats %s %s" % (Utils.VACClientPath, self.endpointArgs, contract, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getVACAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getVACAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.VACClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getVACBalances([source] + accounts)
    ...
    def getVACBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountVACBalance(account.name)
    ...
    def getAccountsByKey(self, key):
        cmd="%s %s get accounts %s" % (Utils.VACClientPath, self.endpointArgs, key)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    # Get actions mapped to an account (clVAC get actions)
    def getActions(self, account, pos=-1, offset=-1):
        ...
        if not self.enableMongo:
            cmd="%s %s get actions -j %s %d %d" % (Utils.VACClientPath, self.endpointArgs, account.name, pos, offset)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
    ...
    def getServants(self, name):
        cmd="%s %s get servants %s" % (Utils.VACClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccountVACBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clVAC get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("VACio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountVACBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountVACBalance(self, scope):
        """Returns SYS currency0000 account balance from clVAC get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountVACBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.VACClientPath, self.endpointArgs, account)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def publishContract(self, account, contractDir, wastFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.VACClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.VACClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False):
        cmd="%s %s set action permission -j %s %s %s %s" % (
            Utils.VACClientPath, self.endpointArgs, account, code, pType, requirement)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Utils.runCmdReturnJson(cmd)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False):
        ...
        cmd="%s %s %s -j %s %s \"%s %s\" \"%s %s\" %s" % (
            Utils.VACClientPath, self.endpointArgs, specificCmd, fromAccount.name, toAccount.name, netQuantity, CORE_SYMBOL, cpuQuantity, CORE_SYMBOL, transferStr)
    ...
    def regproducer(self, producer, url, location, waitForTransBlock=False):
        ...
        cmd="%s %s %s -j %s %s %s %s" % (
            Utils.VACClientPath, self.endpointArgs, specificCmd, producer.name, producer.activePublicKey, url, location)
        return self.processCmd(cmd, specificCmd, waitForTransBlock)

    def vote(self, account, producers, waitForTransBlock=False):
        specificCmd = "system voteproducer prods"
        cmd="%s %s %s -j %s %s" % (
            Utils.VACClientPath, self.endpointArgs, specificCmd, account.name, " ".join(producers))
        return self.processCmd(cmd, specificCmd, waitForTransBlock)
    ...
    def getInfo(self, silentErrors=False):
        cmd="%s %s get info" % (Utils.VACClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clVAC get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
