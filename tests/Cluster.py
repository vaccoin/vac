...
class Cluster(object):
    ...
    # walletd [True|False] Is kVACd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="VACtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kVACd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: VAC server host
        port: VAC server port
        walletHost: VAC wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures VAC mongo plugin
        ...
        self.VACioAccount=self.defProducerAccounts["VACio"]= Account("VACio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.VACServerName))
            return True
        ...
        if not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodVAC default port.")
            return False

        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.VACLauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodVACArgs="--max-transaction-time 5000 --abi-serializer-max-time-ms 5000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodVACArgs += " --plugin VACio::wallet_api_plugin"
        if self.enableMongo:
            nodVACArgs += " --plugin VACio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if Utils.Debug:
            nodVACArgs += " --contracts-console"

        if nodVACArgs:
            cmdArr.append("--nodVAC")
            cmdArr.append(nodVACArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.VACServerName, totalNodes, len(nodes)))
            return False
        ...
        self.VACioAccount=self.defProducerAccounts["VACio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.VACClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key" % (Utils.VACClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on VAC server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextVACIdx: %d, n: %d" % (nextVACIdx, n))
                nextVACIdx=(nextVACIdx + 1)%count
                if not self.nodes[nextVACIdx].killed:
                    #Utils.Print("nextVACIdx: %d" % (nextVACIdx))
            ...
            #Utils.Print("nextVACIdx: %d, count: %d" % (nextVACIdx, count))
            node=self.nodes[nextVACIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.VACServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on VAC node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getVACBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.VACioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseProducers(nodeNum):
        ...
        configFile="etc/VACio/%s/config.ini" % (node)
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse VACio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            VACioName="VACio"
            VACioKeys=producerKeys[VACioName]
            VACioAccount=Account(VACioName)
            VACioAccount.ownerPrivateKey=VACioKeys["private"]
            VACioAccount.ownerPublicKey=VACioKeys["public"]
            VACioAccount.activePrivateKey=VACioKeys["private"]
            VACioAccount.activePublicKey=VACioKeys["public"]

            if not walletMgr.importKey(VACioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (VACioName))
                return False
            ...
            trans=biosNode.publishContract(VACioAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, VACioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        myTrans=biosNode.pushMessage("VACio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("VACio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but VACio).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "VACio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            VACioTokenAccount=copy.deepcopy(VACioAccount)
            VACioTokenAccount.name="VACio.token"
            trans=biosNode.createAccount(VACioTokenAccount, VACioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (VACioTokenAccount.name))
                return False

            VACioRamAccount=copy.deepcopy(VACioAccount)
            VACioRamAccount.name="VACio.ram"
            trans=biosNode.createAccount(VACioRamAccount, VACioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (VACioRamAccount.name))
                return False

            VACioRamfeeAccount=copy.deepcopy(VACioAccount)
            VACioRamfeeAccount.name="VACio.ramfee"
            trans=biosNode.createAccount(VACioRamfeeAccount, VACioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (VACioRamfeeAccount.name))
                return False

            VACioStakeAccount=copy.deepcopy(VACioAccount)
            VACioStakeAccount.name="VACio.stake"
            trans=biosNode.createAccount(VACioStakeAccount, VACioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (VACioStakeAccount.name))
                return False
            ...
            contract="VACio.token"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(VACioTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False

            # Create currency0000, followed by issue currency0000
            contract=VACioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (VACioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to VACio contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (VACioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to VACio contract.")
                return False
            ...
            Utils.Print("Verify VACio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountVACBalanceStr(VACioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return False

            contract="VACio.system"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (VACioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (VACioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (VACioTokenAccount.name, name))
                    return False
    ...
    # Populates list of VACInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.VACServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.VACServerName, pattern))
                break
    ...
    # Kills a percentange of VAC instances starting from the tail and update VACInstanceInfos state
    def killSomeVACInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.VACServerName, killSignal))
    ...
    def dumpErrorDetails(self):
        fileName="etc/VACio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/VACio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodVAC instances. allInstances will kill all nodVAC instances running on the system."""
        cmd="%s -k 9" % (Utils.VACLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down VAC cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the VAC server
            cmd="pkill -9 %s" % (Utils.VACServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down VAC cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/VACio/node_*"):
            shutil.rmtree(f)
