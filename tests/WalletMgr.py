...
class WalletMgr(object):
    __walletLogFile="test_kVACd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kVACd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodVACPort=8888, nodVACHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodVACPort=nodVACPort
        self.nodVACHost=nodVACHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodVACHost, self.nodVACPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kVACd")
            return False

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d --verbose-http-errors" % (
            Utils.VACWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
    ...
    def create(self, name, accounts=None):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s" % (Utils.VACClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def importKey(self, account, wallet, ignoreDupKeyWarning=False):
        ...
        cmd="%s %s wallet import --name %s --private-key %s" % (
            Utils.VACClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s  --private-key %s" % (
                Utils.VACClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.VACClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.VACClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.VACClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.VACClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.VACClientPath, self.endpointArgs, wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    def killall(self, allInstances=False):
        """Kill kVAC instances. allInstances will kill all kVAC instances running on the system."""
        if self.__walletPid:
            Utils.Print("Killing wallet manager process %d" % (self.__walletPid))
            os.kill(self.__walletPid, signal.SIGKILL)

        if allInstances:
            cmd="pkill -9 %s" % (Utils.VACWalletName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
