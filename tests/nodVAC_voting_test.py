...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.VACioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    if testWallet is None:
        Utils.cmdError("VAC wallet create")
    ...
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.VACioAccount.name))
        trans=node.createInitializeAccount(account, cluster.VACioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000)
        if trans is None:
            Utils.cmdError("%s create account" % (account.name))
            errorExit("Failed to create account %s" % (account.name))
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.VACioAccount.name, account.name))
        if node.transferFunds(cluster.VACioAccount, account, transferAmount, "test transfer") is None:
            errorExit("Failed to transfer funds %d from account %s to %s" % (
                transferAmount, cluster.VACioAccount.name, account.name))
