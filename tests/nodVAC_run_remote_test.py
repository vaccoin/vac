...
###############################################################
# nodVAC_run_remote_test
#  Tests remote capability of the nodVAC_run_test. Test will setup cluster and pass nodes info to nodVAC_run_test. E.g.
#  nodVAC_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodVAC_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodVAC", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killVACInstances, False, False, killAll, dumpErrorDetails)
