/**
 *  @file
 *  @copyright defined in VACio/LICENSE.txt
 */
...
{
   try {
      app().set_version(VACio::nodVAC::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "VACio/nodVAC/data" );
      app().set_default_config_dir(root / "VACio/nodVAC/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodVAC version ${ver}", ("ver", VACio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("VACio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
