...
#include <net/if.h>
#include <VACio/chain/genesis_state.hpp>
...
void
VACd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string VACio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), VACio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (VACd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (VACd_def &node, host_def *host);
  void launch (VACd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, VACd_def> find_node(uint16_t node_num);
  vector<pair<host_def, VACd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodVAC does not require transaction signatures.")
    ("nodVAC", bpo::value<string>(&VACd_extra_args), "forward nodVAC command line argument(s) to each instance of nodVAC, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodVAC instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &VACd : binding.instances) {
          VACd.host = binding.host_name;
          VACd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(VACd.p2p_port);

          aliases.push_back (VACd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("VACIO_HOME");
...
void
launcher_def::assign_name (VACd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.VACio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      VACd_def VACd;
      assign_name(VACd, i == 0);
      aliases.push_back(VACd.name);
      VACd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(VACd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->VACio_home =
          (local_id.contains (lhost->host_name) || servers.default_VACio_home.empty()) ?
          erd : servers.default_VACio_home;
        host_ndx++;
      } // ph_count == 0

      VACd_def VACd;
      assign_name(VACd, do_bios);
      VACd.has_db = false;
      ...
      aliases.push_back(VACd.name);
      VACd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(VACd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  VACd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->VACio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->VACio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->VACio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->VACio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->VACio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = VACio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = VACio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = VACio::net_plugin\n";
  } else {
    cfg << "plugin = VACio::bnet_plugin\n";
  }
  cfg << "plugin = VACio::chain_api_plugin\n"
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      VACio::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (VACd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->VACio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->VACio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->VACio_home;

  cmd = "cd " + host->VACio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->VACio_home
...
void
launcher_def::launch (VACd_def &instance, string &gts) {
  ...
  string VACdcmd = "programs/nodVAC/nodVAC ";
  if (skip_transaction_signatures) {
    VACdcmd += "--skip-transaction-signatures ";
  }
  if (!VACd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +VACio::mongo_db_plugin");
       string args = std::regex_replace (VACd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       VACdcmd += args + " ";
    }
    else {
       VACdcmd += VACd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    VACdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  VACdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  VACdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    VACdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->VACio_home + "; nohup " + VACdcmd + " > "
    ...
    string cmd = "cd " + host->VACio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << VACdcmd << endl;

    bp::child c(VACdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << VACdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(VACd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, VACd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, VACd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const VACd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.VACio_home + "; "
                 + "export VACIO_HOME=" + host.VACio_home + string("; ")
                 + "export VACIO_TN_NODE=" + node_num + "; "
                 + "./scripts/VACio-tn_bounce.sh";
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.VACio_home + "; "
                 + "export VACIO_HOME=" + host.VACio_home + "; "
                 + "export VACIO_TN_NODE=" + node_num + "; "
         + "export VACIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/VACio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the VACio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the VACio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the VACio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << VACio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(VACio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (VACio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_VACio_home) (ssh) )
